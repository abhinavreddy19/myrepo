...
function sign_it() {
  ...
  // process.env.PRIV_KEY2
  let signature = ec.sign(msgHash, process.env.PRIV_KEY2, "hex", {
    canonical: true,
  });
...
register().then((res) => {
  if (res) {
    ...
    client.subscribe("gateway2/priv", (err) => {
      if (!err)
...
sample.events
  ...
  .on("data", async (eve) => {
    ...
    let msg = await sample.methods
      .getMessage()
      .call({ from: process.env.address2 });
