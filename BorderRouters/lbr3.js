...
async function create_transaction(func, str, args) {
  try {
    const tx = func(...args);
    const gas = await tx.estimateGas({ from: process.env.address3 });
    const gasPrice = await web3.eth.getGasPrice();
    const data = tx.encodeABI();
    const nonce = await web3.eth.getTransactionCount(process.env.address3);
...
function decrypt(rcv) {
  ...
  data = ecies.decrypt(process.env.PRIV_KEYX3, cipher).toString();
...
function sign_it() {
  ...
  // process.env.PRIV_KEY3
  let signature = ec.sign(msgHash, process.env.PRIV_KEY3, "hex", {
    canonical: true,
  });
...
const register = async () => {
  console.log("========= WELCOME TO LBR 3 =========\n");
  var hash = web3.utils.soliditySha3("Register");
  var sign = web3.eth.accounts.sign(hash, process.env.PRIV_KEY3);
...
register().then((res) => {
  if (res) {
    ...
    client.subscribe("gateway3/priv", (err) => {
      if (!err)
        console.log("-----Listening for private keys from devices-----");
    });
    client.subscribe("gateway3/register", (err) => {
      if (!err)
        console.log(
          "-----Listening for registration requests from devices-----"
        );
    });
    client.subscribe("gateway3/nonce", (err) => {
      if (!err) console.log("-----Ready to send nonces-----");
    });
    client.subscribe("gateway3/auth", (err) => {
      if (!err)
...
sample.events
  .receive_message({
    filter: { gateway: [process.env.address3] },
    fromBlock: "latest",
  })
  .on("data", async (eve) => {
    console.log(
    ...
    let msg = await sample.methods
      .getMessage()
      .call({ from: process.env.address3 });
...
client.on("message", async (topic, rcv) => {
  ...
  if (topic === "gateway3/register") {
    ...
    let curCount = await web3.eth.getTransactionCount(process.env.address3);
