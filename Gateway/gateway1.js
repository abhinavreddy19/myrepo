...
function aggregateSignatures(signatures) {
  ...
  const combinedSig = hashedSigs.reduce((acc, curr) => acc + curr.toString('hex'), '');
...
async function signAggregated(msg, privateKey) {
  const nonce = await web3.eth.getTransactionCount(getAddressFromPrivateKey(privateKey), 'pending');
  const gasPrice = await web3.eth.getGasPrice();
    ...
    if (topic === 'gateway1/register') {
        ...
        */
        ...
        if (ans.error) {
            ...
            msg.id=data.devId;
            console.log(msg);
            console.log("Devices under gateway :",devices);
            if(devices.length!=0){
